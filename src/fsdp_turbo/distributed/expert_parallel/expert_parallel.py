...
def expert_parallelize_modules(modules: torch.nn.Module, ep_mesh: DeviceMesh, plan: EPPlanConfig):
    ...
    for module in ep_modules:
        ...
        # distribute experts weights unless the meta loader already loaded only
        # this EP rank's local expert shard.
...
def distribute_expert_weight(module_name, module, ep_mesh):
    for name, param in module.named_parameters(recurse=False):
        if isinstance(param, DTensor):
            if param.device_mesh.size() != ep_mesh.size():
                raise ValueError(
                    f"Cannot reinterpret DTensor expert shard from mesh {param.device_mesh} to EP mesh {ep_mesh}. "
                    f"Use ep_size equal to the FSDP shard size or shard experts before FSDP."
                )
            dist_param = torch.nn.Parameter(param.to_local())
        else:
            dist_param = torch.nn.Parameter(distribute_tensor(param, ep_mesh, [Shard(0)]).to_local())
