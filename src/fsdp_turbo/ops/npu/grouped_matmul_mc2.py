...
class AllToAllGroupedMatmul(torch.autograd.Function):
    ...
    def forward(ctx, inputs, weights, group, send_counts, recv_counts, shared_inputs, shared_weight):
        ...
        ctx.save_for_backward(weights, shared_inputs, shared_weight, output)
        ctx.hcomm = hcomm
...
def all2all_grouped_matmul_npu(inputs, weights, group, send_counts, recv_counts, shared_inputs=None, shared_weight=None):
...
def grouped_matmul_all2all_npu(inputs, weights, group, send_counts, recv_counts, shared_inputs=None, shared_weight=None):
